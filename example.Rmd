...
Here we should modify `tidy` function. `tidy.anova()` is not defined for `anova.glm`, so we should add some lines against `warning messages`.
...
This function would be used later to construct the independence model.
...
Function can be defined for more general usage. This return `tibble` with list element. Each list has `glm` object.
