...
author: |
  | Young-geun Kim
  | [dudrmshub@gmail.com](mailto:dudrmshub@gmail.com)
...
Here we should modify `tidy` function of @broom. `tidy.anova()` is not defined for `anova.glm`, so we should add some lines against `warning messages`.
...
This function would be used later to construct the various independence models.
...
Function can be defined for more general usage. This return `tibble` with list element. Each list has `glm` object with `[[1]]`
...
`test = "LRT"` option of `anova.glm()` produces $G^2$. As noticed, this function _is designed to be_ applied with `purrr::map()`.
